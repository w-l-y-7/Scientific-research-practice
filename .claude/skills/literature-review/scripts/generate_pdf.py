...
# 中文 Windows 上 subprocess 默认用 GBK 解码子进程输出，而 pandoc 和 xelatex 吐的是 UTF-8，
# 于是读输出时会抛 UnicodeDecodeError。偏偏这错误最可能在 PDF 生成失败时触发——
# 那正是你最需要看到 pandoc 报错信息的时候。显式指定编码，坏字节用替换字符兜住。
...
def generate_pdf(
    ...
    try:
        ...
        result = subprocess.run(cmd, capture_output=True, check=True, **SUBPROCESS_TEXT)
...
def check_dependencies():
    ...
    if missing:
        ...
        for dep in missing:
            if dep == 'pandoc':
                print("  - pandoc: Windows 用 winget 安装，或去 pandoc.org 下载安装包")
            elif dep == 'xelatex':
                print("  - xelatex: Windows 装 MiKTeX（https://miktex.org/）或 TeX Live")
        return False
