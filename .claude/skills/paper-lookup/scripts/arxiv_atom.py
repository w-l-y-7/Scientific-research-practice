...
throttling -- which it signals with the bare plain-text body `Rate exceeded.`, not a feed.
    ...
    curl.exe -s "https://export.arxiv.org/api/query?id_list=1706.03762" | python arxiv_atom.py -
    python arxiv_atom.py feed.xml --ids-only
"""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        ...
        epilog='curl.exe -s "https://export.arxiv.org/api/query?id_list=1706.03762" | %(prog)s -',
    )
